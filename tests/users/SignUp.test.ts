...
describe('Users - SignUp', () => {
  ...
  it('Should not create a user with duplicated email', async () => {
    const user1: Omit<IUser, 'id' | 'createdAt' | 'updatedAt'> = {
      email: 'binho_alisson25@hotmail.com',
      name: 'Alisson Souza',
      password: '1234567',
    };

    const user2: Omit<IUser, 'id' | 'createdAt' | 'updatedAt'> = {
      email: 'binho_alisson25@hotmail.com',
      name: 'Alisson Souza',
      password: '1234567',
    };
  ...
  it('Should not create a user without email', async () => {
    const user: Omit<IUser, 'id' | 'createdAt' | 'updatedAt' | 'email'> = {
      name: 'Alisson',
      password: '1234567',
    };
  ...
  it('Should not create a user without name', async () => {
    const user: Omit<IUser, 'name' | 'id' | 'createdAt' | 'updatedAt'> = {
  ...
  it('Should not create a user without password', async () => {
    const user: Omit<IUser, 'id' | 'password' | 'createdAt' | 'updatedAt'> = {
      name: 'Alisson',
      email: 'alisson@hotmail.com',
    };
